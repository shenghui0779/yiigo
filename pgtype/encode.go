...
func appendEncodedText(ps *parameterStatus, buf []byte, x interface{}) []byte {
	...
	case []byte:
		encodedBytea := encodeBytea(ps.serverVersion, v)
...
const (
	infinityTsEnabledAlready        = "pgtype: infinity timestamp enabled already"
	infinityTsNegativeMustBeSmaller = "pgtype: infinity timestamp: negative value must be smaller (before) than positive"
)
